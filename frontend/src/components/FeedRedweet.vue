...
export default {
  ...
  created() {
    for (let i = 0; i < this.redweetOf.media.length; i++) {
      let type = "";
      switch (this.redweetOf.media[i].replace(/\?.+/gi, "").split(".").pop().toLowerCase()) {
        case "jpg":
          type = "image/jpeg"
          break;
        case "jpeg":
      ...
      let thumbnailUrl = this.redweetOf.media[i].replace("/o/media", "/o/thumb").replace(/\.(mp4|jpeg|jpg|gif)/gi, ".png").replace(/\?.+/gi, "?alt=media");

      this.thumbList.push({
        url: thumbnailUrl,
        type: type,
        original: this.redweetOf.media[i].replace('storage.googleapis.com/download/storage/v1/', 'firebasestorage.googleapis.com/v0/'),
      });
