...
func ConnectDB() {
	consts.Client = db.NewClient()
	if err := consts.Client.Prisma.Connect(); err != nil {
		panic(err)
	}

	consts.BaseCtx = context.Background()
}

// Disconnect from DB
func DisconnectDB() {
	if err := consts.Client.Prisma.Disconnect(); err != nil {
		panic(err)
	}
}
...
func NewFollower(followedID string, followerID string) (*db.UserModel, error) {
	...
	myUser, err := consts.Client.User.FindUnique(
...
func DeleteDweet(postID string) (*db.DweetModel, error) {
	...
	dweet, err := consts.Client.Dweet.FindUnique(
		db.Dweet.ID.Equals(postID),
	).With(
	...
	// _, err = consts.Client.Prisma.ExecuteRaw(delQuery, post.DbID).Exec(consts.BaseCtx)
...
func DeleteRedweet(postID string, userID string) (*db.RedweetModel, error) {
	...
	_, err = consts.Client.Redweet.FindUnique(
